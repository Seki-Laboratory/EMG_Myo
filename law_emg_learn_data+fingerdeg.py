...
class Emg(myo.DeviceListener):
  ...
  def on_emg(self,event):
    SAVE_DATA_DIR_PATH = "C:/Users/usui0/Desktop/2021_sekilab_data/law/"
    JES = "9"
    ...
    # print(gemg)
    if self.i <= self.n-1:
        with open(SAVE_DATA_DIR_PATH+JES+"/"+JES+'_emgdata'+str(50)+'.csv', 'a') as f:
            writer = csv.writer(f, lineterminator='\n') # 行末は改行
            law_emg = np.append(gemg,self.label)
            law_emg = np.append(law_emg,self.value)
